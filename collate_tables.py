...
#input species name here for now
subprocess.run(["bash", "setup.sh", "salmonella_typhi_ciprofloxacin_h58"])

#sample names
#will have to be adapted for snakemake samples input
sample_names = os.listdir("gfa/")
sample_names = [os.path.splitext(name)[0] for name in sample_names]
sample_df = pd.DataFrame(sample_names, columns=["sample"])
#loading and prepping dataframes

#gplas2
gplas_frames = {}
#add a column populated with sample name (from the file name) to each gplas dataframe individually
#will have to be adapted for snakemake samples input
for tab in os.listdir("gplas"):
    filepath = os.path.join("gplas", tab)
    gplas_frames[tab] = pd.read_csv(filepath, sep= " ")
    
#remove extension from sample names
for k,frame in gplas_frames.items():
    frame["Sample"] = k.split("_")[0]
    gplas_frames[k]=frame


#adding node number to gplas dataframe based on matching contig in .gfa file
for k, frame in gplas_frames.items():
    gplas_frames[k] = frame
    df = pd.DataFrame.from_dict(frame).iloc[0]["Sample"]
    with open("gfa/" + df + ".gfa", "r") as f:
        lines = f.readlines()
...
#abricate csv
ncbi = pd.read_csv("abricate/ncbi.tab", sep="\t")
resfinder = pd.read_csv("abricate/resfinder.tab", sep="\t")
plasmidfinder = pd.read_csv("abricate/plasmidfinder.tab", sep="\t")
#cleaning up #FILE column
...
#checkm
checkm = pd.read_csv("checkm/report.tsv", sep="\t")
